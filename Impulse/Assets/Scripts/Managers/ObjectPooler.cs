...
namespace Managers
{
	public class ObjectPooler : MonoBehaviour
	{
		...
		public void Initialize() {
			...
			if (debug) {
				print("Pooling has ended, "   + _pooledGameObjects.Count + " Pooled Objects");
				print("Generating Pool Took " + (Time.realtimeSinceStartup - timer));
			}
		}
		...
		public GameObject GetGameObject(int index) {
			...
			for (int i = startPosInList; i < startPosInList + gameObjectsToBePooled[index].amountToBePooled; i++) {
				if (!_pooledGameObjects[i].activeSelf) {
					if (debug) print("Object " + _pooledGameObjects[i].name + " Found at position " + i);
			...
			if (gameObjectsToBePooled[index].loadMoreIfNoneLeft) {
				...
				_pooledGameObjects.Insert(startPosInList + gameObjectsToBePooled[index].amountToBePooled,
					gameObjectsToBePooled[index].gameObjectToBePooled);
	...
	{
		[Header("Object Info")] public string name;
		public int amountToBePooled;
		public GameObject gameObjectToBePooled;

		[Header("Settings")] public bool loadMoreIfNoneLeft;
	}
